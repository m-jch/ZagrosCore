...
@section('menu-mid')
    <li class="active"><a>{{trans('ZagrosCore::layout.bug')}}</a></li>
@stop
...
@section('content')
    <div class="col-md-12">
        ...
        <div class="col-md-6">
            <div class="col-md-6">
                <table class="table table-clear">
                    <tbody>
                        <tr>
                            <td><b>{{trans('ZagrosCore::layout.registred_by')}}<b></td>
                            <td>{{$project->milestone->bug->user_created->name}}</td>
                        </tr>
                        <tr>
                            <td><b>{{trans('ZagrosCore::layout.assign_to')}}</b></td>
                            <td>{{isset($project->milestone->bug->user_assigned->name) ? $project->milestone->bug->user_assigned->name : trans('ZagrosCore::layout.not_assigned')}}</td>
                        </tr>
                        <tr>
            ...
            <div class="col-md-6">
                <table class="table table-clear">
                    <tbody>
                        <tr>
                            <td><b>{{trans('ZagrosCore::layout.importance')}}<b></td>
            ...
            <div class="col-md-12">
                @if (Auth::user()->is_admin)
                    <a class="pull-right btn btn-default" href="{{URL::action('MilestoneController@getUpdateBug', array($project->url, $project->milestone->url, $project->milestone->bug->bug_id))}}">{{trans('ZagrosCore::layout.update')}}</a>
                @endif
            </div>
            <div class="col-md-12">
                @foreach ($project->milestone->bug->events as $event)
                    <div class="event">
                        ...
                        @if (!empty($event->changes))
                            <p><b>{{trans('ZagrosCore::layout.changes')}}</b></p>
                            <ul class="changes">
                                {{$event->changes}}
                            </ul>
                        @endif
                        @if (!empty($event->description))
                            <p><b>{{trans('ZagrosCore::layout.desc')}}</b></p>
        ...
        <div class="col-md-6">
            <h4>{{trans('ZagrosCore::layout.blueprint_parent')}}</h4>
            <ul>
                ...
                @else
                    <li>{{trans('ZagrosCore::layout.no_parent')}}</li>
                @endif
            </ul>
        </div>
    </div>
